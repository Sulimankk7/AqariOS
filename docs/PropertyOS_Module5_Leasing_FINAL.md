...
- Once superseded, a contract's financial-term columns (`monthly_rent_amount`, `security_deposit_amount`, `start_date`, `end_date`, `payment_frequency`, `payment_due_day`, `currency`) are immutable at the application layer — enforced by application logic checking `EXISTS (SELECT 1 FROM lease_contracts WHERE prior_contract_id = :this_id)` before permitting any UPDATE to those specific columns; a database-level `BEFORE UPDATE` trigger performing the identical check is the recommended production hardening (documented for Phase 8) so the guarantee holds even against a direct SQL write that bypasses application code.
- `building_id` is written once at INSERT time (derived from `apartment_id`) and never independently updated — if an apartment is ever administratively reassigned to a different floor/building (a rare data-correction scenario), that is a correction to the `apartments` row, not to any historical `lease_contracts` row, which correctly continues to reflect the building the lease was actually signed against.

**Soft Delete Strategy:** Standard, but exceptionally rare in practice — a lease contract is a legal financial record; `deleted_at` here is reserved for genuine data-entry errors (a contract created by mistake and never signed/active), not for normal lifecycle transitions, which are expressed through `status`, not deletion. A `draft` contract may reasonably be soft-deleted; an `active` or `superseded` contract should, as a matter of operational policy, never be.
