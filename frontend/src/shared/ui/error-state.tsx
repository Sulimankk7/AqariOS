...
export function ErrorState({ title, description, retryLabel, onRetry, className, ...props }: ErrorStateProps) {
  ...
  return (
    <div role="alert" className={cn("flex min-h-48 w-full flex-col items-center justify-center gap-4 rounded-md bg-error-container/55 px-6 py-10 text-center", className)} {...props}>
      ...
      {onRetry && <Button variant="outlined" size="sm" onClick={onRetry}><RefreshCw />{retryLabel ?? t("common.retry")}</Button>}
    </div>
  );
}
