        ...
        CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            using (var eligibilityScope = _serviceProvider.CreateScope())
            {
                ...
                batchTransactionIds = await dbContext.ExecuteInTransactionAsync(async (ct) =>
                {
                    var transactionRepo = eligibilityScope.ServiceProvider
                        .GetRequiredService<IEfawateercomTransactionRepository>();

                    return await transactionRepo.GetStaleNonTerminalTransactionIdsAsync(
                        staleCutoff, batchSize, afterTransactionId, ct);
                }, cancellationToken);
            }
